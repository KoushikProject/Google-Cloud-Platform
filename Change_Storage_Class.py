#You need storage.buckets.get,storage.objects.create,storage.objects.delete,storage.objects.get,storage.objects.list,storage.objects.update
...
def delete_file_by_update_time(bucket_name,update_time):
    ...
    for blob in blobs:
        if blob.updated <= update_time:
            ...
            print('Storage class changed successfully.')


bucket_name = 'gcp-dev-02'

#Create a datetime object for comparison
ist_dt = datetime(2024, 3, 14, 23, 0, 0, tzinfo=pytz.timezone('Asia/Kolkata'))
